...
function _getBasePath() {
	...
	return '/assets/kindeditor/';  // modify by happy
}
// K.basePath = _getBasePath(); // modify by happy
...
function _formatUrl(url, mode, host, pathname) {
	...
	host = host || K.marchUploadHost || location.protocol + '//' + location.host;
...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData, pluginsPath) {
	var arr = [
		(_direction === '' ? '<html>' : '<html dir="' + _direction + '">'),
		'<head><meta charset="utf-8" /><title></title>',
    // '<script src="' + pluginsPath + '/code/prettify.js?body=1" type="text/javascript"></script>',
    '<link href="' + themesPath + '/common/iframe.css" media="all" rel="stylesheet" type="text/css" />'
	];
	if (!_isArray(cssPath)) {
		cssPath = [cssPath];
	}
...
_extend(KEdit, KWidget, {
	init : function(options) {
      ...
      pluginsPath = _undef(options.pluginsPath, ''),
		...
		function ready() {
			...
			doc.write(_getInitHtml(themesPath, bodyClass, cssPath, cssData, pluginsPath));
			doc.close();
...
function _create(expr, options) {
	options = options || {};
  K.marchUploadHost = options.marchUploadHost;
